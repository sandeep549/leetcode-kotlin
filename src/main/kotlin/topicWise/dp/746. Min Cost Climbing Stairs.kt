...
private fun minCostClimbingStairs(cost: IntArray): Int {
    ...
    fun cost(n: Int): Int { // calculates price to reach at step n
        ...
        if (dp[n] == 0) {
            dp[n] = minOf(cost(n - 1), cost(n - 2)) + cost[n]
        }
        return dp[n]
    }
    cost(cost.size - 1)
    return minOf(dp[cost.size - 1], dp[cost.size - 2]) // to cross last-step(n-1)
}
...
private fun minCostClimbingStairs2(cost: IntArray): Int {
    ...
    for (i in 1..cost.lastIndex) {
        var curr = minOf(prepre, pre) + cost[i]
        prepre = pre
        pre = curr
    }
    return minOf(prepre, pre)
}
